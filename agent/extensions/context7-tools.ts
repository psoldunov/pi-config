...
const ResolveParams = Type.Object({
	libraryName: Type.String({
		description: "Library name to search for (e.g., 'react').",
	}),
	query: Type.String({
...
const DocsParams = Type.Object({
	libraryId: Type.String({
		description:
			"Context7 library ID, e.g., '/facebook/react' or '/vercel/next.js'.",
	}),
	query: Type.String({
...
async function fetchLibraryDocs(
	...
	if (!response.ok) {
		throw new Error(
			`Docs fetch failed: ${response.status} ${response.statusText}`,
		);
	}
...
export default async function (pi: ExtensionAPI) {
	if (!CONTEXT7_API_KEY) {
		console.error(
			"[Context7] CONTEXT7_API_KEY not set. Anonymous tier rate limits apply.",
		);
	}
	...
	pi.registerTool<typeof ResolveParams, ResolveDetails>({
		...
		async execute(_toolCallId, params) {
			try {
				...
				if (!top) {
					return {
						content: [
							{
								type: "text",
								text: `No library matched '${params.libraryName}'.`,
							},
						],
						details: {},
					};
				...
				return {
					content: [
						{ type: "text", text: `Error resolving library ID: ${msg}` },
					],
					details: { error: msg },
				};
			}
	...
	pi.registerTool<typeof DocsParams, DocsDetails>({
		...
		async execute(_toolCallId, params) {
			try {
				...
				const content = await fetchLibraryDocs(
					params.libraryId,
					params.query,
					tokens,
				);
	...
	pi.registerTool<typeof CachedParams, CachedDetails>({
		...
		async execute(_toolCallId, params) {
			const tokens = params.tokens ?? DEFAULT_DOC_TOKENS;
			if (params.query) {
				const exact = docCache.get(
					`${params.libraryId}::${params.query}::${tokens}`,
				);
				if (exact) {
					return {
			...
			const entries = [...docCache.entries()].filter(([k]) =>
				k.startsWith(prefix),
			);
			if (entries.length === 0) {
				return {
					content: [
						{
							type: "text",
							text: `No cached documentation found for '${params.libraryId}'.`,
						},
					],
